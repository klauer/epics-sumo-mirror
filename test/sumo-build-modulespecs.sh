...
$SUMO build --db $DEPS --supportdir $SUPPORTDIR --dump-modules try :load:$MODULES
echo
echo "Take modulespecs from build file:"
$SUMO build --db $DEPS --supportdir $SUPPORTDIR --dump-modules try :build:AUTO-001
echo
echo "The same but remove ALARM:R3-7"
$SUMO build --db $DEPS --supportdir $SUPPORTDIR --dump-modules try :build:AUTO-001 :rm:ALARM:R3-7
echo
echo "The same but change ALARM:R3-7 to ALARM:R3-8"
$SUMO build --db $DEPS --supportdir $SUPPORTDIR --dump-modules try :build:AUTO-001 ALARM:R3-8
echo
echo "The same but remove all and add ALARM:R3-8"
$SUMO build --db $DEPS --supportdir $SUPPORTDIR --dump-modules try :build:AUTO-001 :clear ALARM:R3-8
