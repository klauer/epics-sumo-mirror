...
cat samples/DB_IDCP | sed -e 's#R[0-9\.]\+/base#epicsbase#' > $TESTDIR/DB_IDCP

cd $TESTDIR > /dev/null

echo -e "call sumo-build new, let the command fail on purpose..."

$PYTHON ../../bin/sumo-build --arch vxWorks-68040 --arch vxWorks-ppc603 --db DB -P DB_IDCP --builddb BUILDS --no-make new 001 2>&1 | tail -n 1
