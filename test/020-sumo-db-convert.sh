...
if [ "$1" = "deps" ]; then
        echo "$ME.tst: $ME.sh $ME.out $ME.ok 010-sumo-scan-all.tst"
        echo
        exit
fi

# This program uses the scan file hat was created for test 010-sumo-scan-all.
...
echo "DB file:"
cat $EXAMPLEDIR/DEPS.DB | sed -e s#$PWD_NICE##
echo
echo "SCANDB file:"
